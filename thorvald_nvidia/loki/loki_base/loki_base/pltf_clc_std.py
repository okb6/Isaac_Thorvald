...
class PltfClcStd:
    ...
    def initialize(self, BaseDriver, motor_drives):
        ...
        self.drive_inverted = [False, False, False, False]
    ...
    def calc_commands(self, vx, vy, wz, motor_drives, joint_states_out, drive_inverted):
        ...
        if wz != 0:
            ...
            while i < 4:
                ...
                drive_y = self.motor_drives[i]["y"]
                if wz < 0:
                    before_norm = - math.atan2((turn_rad_x - drive_x), (turn_rad_y - drive_y)) + math.pi
                else:
                    before_norm = - math.atan2((turn_rad_x - drive_x), (turn_rad_y - drive_y))

                drive_steer[i] = self.normalize_angle(self, before_norm)
        ...
        #Correct the drive inversions that aren't done in simualtion
        i = 0
        while i < 4:
            if vx < 0 and wz == 0 :
                steering[i] -= math.pi
                ...
                steering[i] += math.pi
                speed[i] *= -1
            elif wz < 0 and vx == 0 and (i == 0 or i == 1):
                if i == 0:
                    steering[i] -= math.pi
                    speed[i] *= -1
                else:
            ...
            elif wz > 0 and vx == 0 and (i == 2 or i == 3):
                if i == 2:
                    steering[i] -= math.pi
                    speed[i] *= -1
                else:
    ...
    def normalize_angle(self, angle):
        a = self.normalize_positive_angle(self, angle)
        if a > math.pi:
            a -= 2.0 * math.pi
        return a


    def normalize_positive_angle(self, angle):
        
        ang = math.fmod(math.fmod(angle, (2.0 * math.pi)) + (2 * math.pi) , (2.0 * math.pi))
        return ang
